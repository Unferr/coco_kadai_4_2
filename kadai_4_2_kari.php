...
if (isset($_POST['send'])) {
  ...
  if (!empty($id)) {
    echo '<a href="../kadai_4_2_login/kadai_4_2_login.php">ログイン画面へ</a><br />';
    echo 'すでに登録されているメールアドレスです。';
  }
  else{
    $userid = uniqid(rand());
    $url_id = hash('sha256',uniqid(rand(),1));
    $url = 'http://co-19-332.99sv-coco.com/coco_kadai_4_2/kadai_4_2_touroku/kadai_4_2_touroku.php'.'?url_id='.$url_id;
    $title = '本登録手続きのお知らせ';
    $message = 
    "ユーザーID\n"
    .$userid
 ...
 if(isset($_SERVER['HTTP_USER_AGENT'])) {
  ...
  if(isset($agentno)) {
    ...
    $smarty -> display("kadai_4_2_kari.tpl");
?>
